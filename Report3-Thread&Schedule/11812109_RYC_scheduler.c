...
struct waitqueue * wait_free = NULL;

void get_name(int pid, char *namebuf){
	FILE *fp;
	char path[BUFLEN];
	char tmp[BUFLEN];
    memset(path, '\0', sizeof(char) * BUFLEN);
	sprintf(path, "/proc/%d/status", pid);

    if((fp=fopen(path,"r")) == NULL){
        return;
    }
	fscanf(fp,"%s %s",tmp, namebuf);
    fclose(fp);
    return;
}
...
void free_select(){
	if (wait_free) {
		if(current == wait_free){
			current = NULL;
		}

		// MAYBE BUG
		wait_free->next = NULL;
		for (int i = 0; (wait_free->job->cmdarg)[i] != NULL; i++) {
			free((wait_free->job->cmdarg)[i]);
			(wait_free->job->cmdarg)[i] = NULL;
		}

		free(wait_free->job->cmdarg);
		free(wait_free->job);
		free(wait_free);
		wait_free = NULL;
	}
...
void jobswitch()
{
	...
	} else if (next != NULL && current != NULL && current != wait_free) { /* do switch */
...
void do_stat()
{
	...
	*/
	...
	if (current) {
		...
		memset(namebuf, '\0', sizeof(char) * BUFLEN);
        get_name(current->job->pid, namebuf);
